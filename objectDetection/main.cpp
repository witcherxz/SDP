...
int main() {
    ...
    ros::NodeHandle n;
    ros::Publisher userInputs = n.advertise<std_msg::bool>("objectFound ", 10);
    ...
    {
        // msg.data = 0;
        // std::cout << "intial objectFound : " << msg.data << std::endl;
        // userInputs.publish(msg);
        // std::cout << "Msg of objectDetection had been published" << std::endl;
        // ros::spinOnce();
        // loop_rate.sleep();

        msg.data = objectDetection(false);
        std::cout << "objectFound: " << msg.data << std::endl;
        userInputs.publish(msg);
        std::cout << "Msg of objectDetection had been published" << std::endl;
        ros::spinOnce();
        loop_rate.sleep();
...
void objectDetection::start(bool showCamera){
    ...
    while(true) {
        ...
        // Show the frame
        if(showCamera) cv::imshow("depth", depthFrameColor);
